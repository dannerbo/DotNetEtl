...
{
	[TestClass]
	public class ParallelizedDataImportTests
	{
		[TestMethod]
		public void Constructor1_DataSourceIsProvided_DataSourcePropertyIsSet()
		{
			...
			var dataImport = new ParallelizedDataImport(dataSource, dataDestination);
		...
		public void Constructor2_DataSourceIsProvided_DataSourcePropertyIsSet()
		{
			...
			var dataImport = new ParallelizedDataImport(dataSource, dataDestinations);
		...
		public void TryRun_ReadableRecordWithRecordMapper_RecordIsMapped()
		{
			...
			var dataImport = new ParallelizedDataImport(dataSource, dataDestination, recordMapper: recordMapper);
		...
		public void TryRun_ReadableRecordWithRecordValidator_RecordIsValidated()
		{
			...
			var dataImport = new ParallelizedDataImport(dataSource, dataDestination, recordValidator: recordValidator);
		...
		public void TryRun_MappableRecordWithRecordValidator_MappedRecordIsValidated()
		{
			...
			var dataImport = new ParallelizedDataImport(dataSource, dataDestination, recordValidator: recordValidator, recordMapper: recordMapper);
		...
		public void TryRun_ReadableRecordWithRecordFormatter_RecordIsFormatted()
		{
			...
			var dataImport = new ParallelizedDataImport(dataSource, dataDestination, recordFormatter: recordFormatter);
		...
		public void TryRun_UnmappableRecordWithRecordFormatter_RecordIsNotFormatted()
		{
			...
			var dataImport = new ParallelizedDataImport(dataSource, dataDestination, recordMapper: recordMapper, recordFormatter: recordFormatter);
		...
		public void TryRun_InvalidRecordWithRecordFormatter_RecordIsNotFormatted()
		{
			...
			var dataImport = new ParallelizedDataImport(dataSource, dataDestination, recordValidator: recordValidator, recordFormatter: recordFormatter);
		...
		public void TryRun_WritableRecordWithAllDependenciesProvided_ImportSucceeds()
		{
			...
			var dataImport = new ParallelizedDataImport(dataSource, dataDestination, recordValidator, recordMapper, recordFormatter);
		...
		public void TryRun_RecordMapperTryMapRecordThrowsException_RollbackOccursAndExceptionIsPropogated()
		{
			...
			dataSource.Stub(x => x.CreateDataReader()).Return(dataReader);
			dataDestination.Stub(x => x.CreateDataWriter(Arg<IDataSource>.Is.Equal(dataSource))).Return(dataWriter);
			dataReader.Stub(x => x.TryReadRecord(out Arg<object>.Out(record).Dummy, out Arg<IEnumerable<FieldFailure>>.Out(null).Dummy)).Return(true).Repeat.Once();
			recordMapper.Stub(x => x.TryMap(
					Arg<object>.Is.Equal(record),
					out Arg<object>.Out(null).Dummy,
					out Arg<IEnumerable<FieldFailure>>.Out(null).Dummy))
		...
		public void TryRun_RecordValidatorTryValidateRecordThrowsException_RollbackOccursAndExceptionIsPropogated()
		{
			...
			dataSource.Stub(x => x.CreateDataReader()).Return(dataReader);
			dataDestination.Stub(x => x.CreateDataWriter(Arg<IDataSource>.Is.Equal(dataSource))).Return(dataWriter);
			dataReader.Stub(x => x.TryReadRecord(
					out Arg<object>.Out(record).Dummy,
					out Arg<IEnumerable<FieldFailure>>.Out(null).Dummy))
				.Return(true)
				.Repeat.Once();
			recordMapper.Stub(x => x.TryMap(
